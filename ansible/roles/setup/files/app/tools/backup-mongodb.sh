...
readonly BACKUP_LOCAL_DIR="/opt/lba/backups/mongodb"
readonly BACKUP_FILE="${BACKUP_LOCAL_DIR}/mongodb-$(date +'%Y-%m-%d_%H%M%S').gpg"

function backup() {
  echo "Creating backup..."
  mkdir -p "${BACKUP_LOCAL_DIR}"
  docker exec lba_mongodb bash -c "mongodump -j=2 --gzip --archive -u backup -p {{ vault.LBA_MONGODB_BACKUP_PASSWORD }}" \
