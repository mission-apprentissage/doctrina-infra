...
readonly BACKUP_LOCAL_DIR="/opt/doctrina/backups/mongodb"
readonly BACKUP_FILE="${BACKUP_LOCAL_DIR}/mongodb-$(date +'%Y-%m-%d_%H%M%S').gpg"

function backup() {
  echo "Creating backup..."
  mkdir -p "${BACKUP_LOCAL_DIR}"
  docker exec doctrina_mongodb bash -c "mongodump --gzip --archive -u backup -p {{ vault.DOCTRINA_MONGODB_BACKUP_PASSWORD }}" \
